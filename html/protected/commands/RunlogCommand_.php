			...
			switch($item['NE_RUN_TYPE_ID']){
				case 1 :
					...
					foreach($arr['title'] as $key=>$val){
						...
						if(count($val) > 0){
							foreach($val as $key2=>$val2){
								$ParentID = self::LoadSQL("SELECT ID FROM ATTRIB_LIST WHERE ATTRIB_TYPE_ID = '".$Parent_Group_Id[$i]."' AND NAME = '".mysql_escape_string($Parent_Group[$i])."'");
								$GroupID = self::LoadSQL("SELECT ID FROM ATTRIB_LIST WHERE ATTRIB_TYPE_ID = '".$Parent_Group_Id[$i+1]."' AND NAME = '".mysql_escape_string($key2)."'");
								$i++;
								if(count($val2) > 0){
									foreach($val2 as $key3=>$val3){
										if(count($val3) > 0){
											foreach($val3 as $key4=>$val4){
												$KeyID = self::LoadSQL("SELECT ID FROM ATTRIB_LIST WHERE NAME = '".mysql_escape_string($val4['key'])."' LIMIT 1");
				...
				case 2 :
					...
					$GroupID = self::LoadSQL("SELECT ID FROM ATTRIB_LIST WHERE ATTRIB_TYPE_ID = '".$IdGroup."' AND NAME = '".mysql_escape_string($arr['head'][0])."'");
							...
							foreach($val as $key2=>$val2){
								if(is_array($val2) > 0){
									foreach($val2 as $key3=>$val3){
										...
										$KeyID = self::LoadSQL("SELECT ID FROM ATTRIB_LIST WHERE ATTRIB_TYPE_ID = '".$id."' AND NAME = '".mysql_escape_string($key3)."'");
									...
									$KeyID = self::LoadSQL("SELECT ID FROM ATTRIB_LIST WHERE ATTRIB_TYPE_ID = '".$id."' AND NAME = '".mysql_escape_string($key2)."'");
					...
					foreach($arr['title'] as $key=>$val){
						...
						foreach($val as $key2=>$val2){
							...
							if($cnt > 1){
								...
								$GroupID = self::LoadSQL("SELECT ID FROM ATTRIB_LIST WHERE ATTRIB_TYPE_ID = '".$IdGroup."' AND NAME = '".mysql_escape_string($key2)."'");
				...
				case 18 :
					...
					foreach($arr['title'] as $key=>$val){
						...
						if(is_array($val)){
							...
							$KeyID = self::LoadSQL("SELECT ID FROM ATTRIB_LIST WHERE ATTRIB_TYPE_ID = '".$id."' AND NAME = '".mysql_escape_string($key)."'");
							$Entry_id = '';
